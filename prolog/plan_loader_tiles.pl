:- module(plan_loader_tiles,
  [ load_plan/1,
    apply_plan/0,
    clear_plan/0,
    solve_plan_tiles/3,          % -Rects,-Oris,-Grid
    solve_plan_tiles_to_file/1,  % +OutFile
    solve_plan_tiles_to_file/2   % +OutFile,+Format(json|pl)
  ]).
...
:- multifile layout_tiles:progress_hook/1.

:- dynamic current_plan_module/1.

default_grid_fallback(100).

/* === совместимая обвязка с существующим планом === */
...
solve_plan_tiles(Rects, Oris, Grid) :-
    current_plan_module(M),
    ( predicate_property(M:task(_,_), defined)
    -> clause(M:task(Count, size(W,H)), true),
       layout_tiles:layout_tiles(Count, W,H, Rects, Oris, Meta),
       Grid = Meta.grid_mm
    ;  throw(error(existence_error(predicate, M:task/2), _))
    ).

solve_plan_tiles_to_file(File) :-
    ( file_name_extension(_, json, File) ->
        solve_plan_tiles_to_file(File, json)
    ;   solve_plan_tiles_to_file(File, pl)
    ).

solve_plan_tiles_to_file(File, Format) :-
    solve_plan_tiles(Rects, Oris, Grid),
    ( Format == json ->
...
/* === мэппинг плана (как в старом загрузчике) === */
detect_plan_module(Abs, M) :-
...
map_object(Id, Unknown, Rect, _Props) :-
    print_message(warning, plan_loader_tiles(unknown_type(Unknown, Id))),
    asserta(zones:forbidden_zone(Id, struct, Rect)).

/* === утилиты для объектов на стене (копии из старого загрузчика) === */
...
window_clear_zone(rect(X,Y,W,H), rect(Xz,Yz,Wz,Hz)) :-
    rects:room_size(Wr,Hr),
    ( zones:policy(window_clear_depth_mm, D) -> true ; D = 1000 ),
    X2 is X+W, Y2 is Y+H,
...
panel_service_zone(rect(X,Y,W,H), rect(Xz,Yz,Wz,Hz)) :-
    rects:room_size(Wr,Hr),
    ( zones:policy(panel_clear_size, size(Along,Depth)) -> true ; (Along=1000,Depth=1000) ),
    X2 is X+W, Y2 is Y+H,
...
netcab_service_zone(rect(X,Y,W,H), rect(Xz,Yz,Wz,Hz)) :-
    rects:room_size(Wr,Hr),
    ( zones:policy(netcab_clear_depth, D) -> true ; D = 1000 ),
    X2 is X+W, Y2 is Y+H,
    ( Y =:= 0   -> Xz is X,     Yz is 0,     Wz is W,  Hz is D
    ; Y2 =:= Hr -> Xz is X,     Yz is Hr-D,  Wz is W,  Hz is D
    ; X =:= 0   -> Xz is 0,     Yz is Y,     Wz is D,  Hz is H
