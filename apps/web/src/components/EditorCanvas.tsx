...
export const EditorCanvas: React.FC = () => {
  ...
  const {
    plan, updateObject, selectedId, setSelected,
    placingType, setPlacingType, addObject,
    deleteSelected, nextIdForType, // ⬅️ короткие ID
  } = usePlanStore();
  ...
  const [ghost, setGhost] = React.useState<{ X: number; Y: number; W: number; H: number } | null>(null);
  ...
  const zoomAt = React.useCallback((sx: number, sy: number, factor: number) => {
    setView(prev => {
      ...
      const baseX = PADDING_PX, baseY = PADDING_PX;
  ...
  React.useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === '0') { e.preventDefault(); setView({ zoom: 1, panX: 0, panY: 0 }); }
      if (e.key.toLowerCase() === 'f') { e.preventDefault(); setView({ zoom: 1, panX: 0, panY: 0 }); }
      if (e.key === 'Escape') { setPlacingType(undefined); setGhost(null); }
      if ((e.key === 'Delete' || e.key === 'Backspace') && !placingType) { e.preventDefault(); deleteSelected(); }
    };
  ...
  const handleMouseMove = (e: any) => {
    if (placingType) { updateGhostAt(); return; }
    if (!isPanning) return;
    const ps = panStart.current; if (!ps) return;
    if (e.evt.buttons === 0) { stopPan(); return; }
    const dx = e.evt.clientX - ps.x, dy = e.evt.clientY - ps.y;
    setView(v => ({ ...v, panX: ps.panX + dx, panY: ps.panY + dy }));
  };
  const handleClick = () => {
    if (!placingType || !ghost) return;
    const id = nextIdForType(placingType); // ⬅️ короткий последовательный id
  ...
  const onTransformLive = (o: StaticObject, node: any) => {
    ...
    const X = Math.round((node.x() - baseX) / mm2px), Y = Math.round((node.y() - baseY) / mm2px);
    setLiveBox({ id: o.id, X, Y, W, H });
  };
  const onDragEnd = (o: StaticObject, xPx: number, yPx: number) => {
    const mmX = Math.round((xPx - baseX) / mm2px), mmY = Math.round((yPx - baseY) / mm2px);
    const snappedX = snap(mmX, GRID_MM), snappedY = snap(mmY, GRID_MM);
    if (o.requiresWallAnchor || o.type === 'window' || o.type === 'door') {
      const distLeft = snappedX, distTop = snappedY, distRight = plan.room.W - (snappedX + o.rect.W), distBottom = plan.room.H - (snappedY + o.rect.H);
      ...
      else if (min === distRight) updateObject(o.id, { X: plan.room.W - o.rect.W, Y: snappedY });
  ...
  const onTransformEnd = (o: StaticObject) => {
    ...
    const scaleX = node.scaleX(), scaleY = node.scaleY();
    let newWmm = Math.max(MIN_SIZE_MM, Math.round((node.width() * scaleX) / mm2px));
    let newHmm = Math.max(MIN_SIZE_MM, Math.round((node.height() * scaleY) / mm2px));
    newWmm = snap(newWmm, GRID_MM); newHmm = snap(newHmm, GRID_MM);
  ...
  const Grid: React.FC = () => {
    ...
    for (let mm = 0; mm <= plan.room.W; mm += GRID_MM) {
      ...
      lines.push(<Line key={'v'+mm} points={[x, baseY, x, baseY + roomHpx]} stroke="#eee" />);
    }
    for (let mm = 0; mm <= plan.room.H; mm += GRID_MM) {
      const y = baseY + mm * mm2px;
      lines.push(<Line key={'h'+mm} points={[baseX, y, baseX + roomWpx, y]} stroke="#eee" />);
    }
    return <>{lines}</>;
  };
  ...
  const trBoundBox = React.useCallback((oldBox: any, newBox: any) => {
  ...
  // ======= Оверлей размеров (оставьте вашу текущую версию) =======
  // ... функция renderMeasurementOverlay из предыдущего шага ...
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: '100%', height: '100%', position: 'relative', border: '1px solid #e5e7eb', borderRadius: 8, boxSizing: 'border-box', overflow: 'hidden' }}
    >
      <Stage
        ref={stageRef}
        ...
        style={{ cursor: placingType ? 'copy' : (isPanning || spacePressed.current ? (isPanning ? 'grabbing' : 'grab') : 'default') }}
      >
        <Layer>
          <Grid />
          ...
          {plan.objects.map(o => {
            ...
            const style = TYPE_COLOR[o.type] || { fill: 'rgba(99,102,241,0.08)', stroke: '#6b7280' };
            const selected = selectedId === o.id;
            const stroke = selected ? '#6366f1' : style.stroke;
            const strokeWidth = selected ? 2 : 1;
            const fill = style.fill;

            // краткая / подробная подпись
            ...
            return (
              <React.Fragment key={o.id}>
                <Rect
                  ref={(ref)=>{ if (ref) shapeRefs.current[o.id] = ref; }}
                  ...
                  onDragEnd={(e)=> onDragEnd(o, e.target.x(), e.target.y())}
        ...
        {/* оставьте ваш верхний слой с измерительным оверлеем */}
